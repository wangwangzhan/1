...
class PrefixSampler:
    ...
    def generate_samples(self, raw_data, batch_size):
        ...
        for batch in range(len(raw_data) // batch_size):
            ...
            rewritten_text = self._sample_from_model(original_text, min_words=30 if self.args.dataset in ['pubmed'] else 55, truncate_ratio=self.args.truncate_ratio)

            for o, s in zip(original_text, rewritten_text):
                if self.args.dataset == 'pubmed':
...
def experiment(args):
    ...
    for idx in tqdm.tqdm(range(n_samples), desc=f"Computing {name} criterion"):
        ...
        # rewritten text
        rewritten_crit = criterion_fn(sampler, rewritten_text)
        # result
    ...
    predictions = {'real': [x["original_crit"] for x in results],
                   'samples': [x["rewritten_crit"] for x in results]}
